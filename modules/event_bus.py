...
使用方法:
    from modules.mod_event_bus import EventBus            # 导入模块
    event_bus = EventBus()                                # 创建/获取实例

    # 订阅事件
    my_queue = queue.Queue()                             # 创建一个自己的事件队列(收件箱)
    event_bus.subscribe("事件类型", my_queue, "订阅人")   # 订阅某种类型的事件

    # 发布事件
    data = {"key1": value1, "key2": value2, ...}         # 准备要发布的事件内容
    event_bus.publish("事件类型", "发布人", data)         # 发布事件
...
class EventBus:
    ...
    def subscribe(self, event_type, event_queue, name = "UNKNOWN"):
        if not isinstance(event_queue, queue.Queue):
            raise TypeError(f'使用 subscribe() 时传入了错误的事件容器')
        ...
        # 打印日志
        logger.info(f'EVENT: 订阅了 "{event_type}" 事件')


    def publish(self, event_type, data = {}, name = "UNKNOWN"):
        # 事件类型转换为大写字符串
        event_type = str(event_type).upper()
        ...
        # 将事件类型,数据,发布人打包进字典
        event = {"type": event_type, "data": data, "name": name}
        ...
        # 打印日志
        if event_type != "UPDATE_LAYER":            # OLED屏幕刷新事件太频繁，跳过
            logger.info(f'EVENT: "{name}" 发布了 "{event_type}" 事件, 内容 = {data}')
