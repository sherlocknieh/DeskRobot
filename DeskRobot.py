...
if __name__ == "__main__":
    ...
    robot.add_thread(OledThread(event_bus,config.get("oled_width", 128), config.get("oled_height", 64), 
                                config.get("oled_i2c_address", 0x3C),
                                config.get("oled_is_simulation", False)))

    # # 表情模块  # 依赖第三方库 : pillow
    from modules.mod_roboeyes import RoboeyesThread
    robot.add_thread(RoboeyesThread(event_bus, config.get("roboeyes_frame_rate", 50), 
                                      config.get("roboeyes_width", 128), config.get("roboeyes_height", 64)))
